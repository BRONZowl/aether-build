...
GOLDEN_SOFT_BASH := [?]Soft_Bash_Case {
	...
	{"aws sts get-caller-identity", .Allow, "aws_sts"},
	{"aws ec2 describe-instances", .Allow, "aws_ec2_describe"},
	{"aws ec2 run-instances", .Ask, "aws_ec2_run"},
	{"gcloud compute instances list", .Allow, "gcloud_list"},
	{"gcloud compute instances create x", .Ask, "gcloud_create"},
	{"az vm list", .Allow, "az_vm_list"},
	{"az vm create", .Ask, "az_vm_create"},
	// HTTP / redis / pacman (table wave)
	{"curl -s https://example.com", .Allow, "curl_get"},
	{"curl -X POST https://example.com", .Ask, "curl_post"},
	{"wget --spider https://example.com", .Allow, "wget_spider"},
	{"wget -O file https://example.com", .Ask, "wget_file"},
	{"redis-cli PING", .Allow, "redis_ping"},
	{"redis-cli SET k v", .Ask, "redis_set"},
	{"pacman -Q", .Allow, "pacman_query"},
	{"pacman -Ss nginx", .Allow, "pacman_search"},
	{"pacman -S nginx", .Ask, "pacman_install"},
	{"poetry show", .Allow, "poetry_show"},
	{"poetry install", .Ask, "poetry_install"},
	{"just --list", .Allow, "just_list"},
	{"just build", .Ask, "just_build"},
	// write-like shell
